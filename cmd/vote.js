...
exports.run = async (bot, msg, params) => {
  ...
  let VOTE_TEXT = await msg.channel.send("Vote now! (30 minutes)");
  await VOTE_TEXT.react(agree);
  await VOTE_TEXT.react(disagree);

  const reactions = await VOTE_TEXT.awaitReactions(reaction => reaction.emoji.name === agree || reaction.emoji.name === disagree, {time: 1800000});
  VOTE_TEXT.delete();
